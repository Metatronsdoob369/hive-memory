...
- **Mission 2V — ASSIGNED TO HERMES (2026-08-13, runs before or parallel to Mission 2): the hostile buyer.** Independent validation of the three live mounts — Joe requires a yes that does not come from Claude. Rules: act as an EXTERNAL agent customer; read ONLY the published contract (live `/.well-known/x402` on localhost:8787, `manifests/openapi.json`, `manifests/mcp-tools.json`) — never spectral-x402 source; do not edit any repo. Buy real (testnet) tiles over BOTH doors with fresh paymentIds; then try to get cheated: replay the same paymentId across doors (must be one receipt, byte-identical), reuse a paymentId with different args (must 409), underpay/wrong-asset/wrong-recipient (must refuse with declared codes, never deliver), bogus + malformed cids (404/400, never a crash), malformed X-Payment (402 refusal), entitlement behavior, and byte-verify every delivery against the pack manifest cids. Attempt at least 3 attacks of your own invention. Deliverable: `2026-08-XX-hostile-buyer-report.md` IN THIS FOLDER — every attempt, expected vs observed, verdict per mount, and an overall BREAKABLE / NOT-BREAKABLE-BY-ME verdict. Written to refute, not to confirm.
